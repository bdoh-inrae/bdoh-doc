...
Cette dualité a une forme logique qu'il vaut la peine de nommer. Un agent est soit un humain soit une machine, deux formes mutuellement exclusives dotées chacune de ses attributs : c'est ce que la théorie des types appelle une somme. Or le modèle relationnel repose sur l'opération inverse, le produit : une relation y est un sous-ensemble d'un produit cartésien de domaines, et une ligne est une conjonction de champs qui coexistent. Le relationnel exprime donc nativement la conjonction de faits, mais ne possède aucune construction native pour la disjonction de formes. Les trois stratégies qui suivent ne sont, de ce point de vue, que trois manières de simuler une somme dans un modèle qui ne connaît que le produit.
...
**Turing** complète ce tableau en montrant que la décidabilité complète est impossible. Aucun schéma ne pourra jamais valider automatiquement toute la sémantique FAIR. Le pipeline de calcul doit par ailleurs être traité comme une entité de plein droit, d'une autre nature qu'une personne : le représenter comme une `Person` aux champs vides est une erreur conceptuelle avant d'être une erreur technique.

**Liskov** énonce en 1987 le principe de substitution : un objet d'un sous-type doit pouvoir remplacer un objet du parent sans altérer les propriétés qu'un programme tient du parent. On l'invoque souvent pour disqualifier TPT, mais il faut être précis sur ce qu'il contraint : il porte sur ce que le parent promet, pas sur ce que les enfants ajoutent. Qu'une machine n'ait pas d'ORCID et qu'une personne pas de numéro de série ne viole rien, ce sont des attributs propres aux enfants. Un parent mince, réduit à ce qui est réellement commun, une identité et le rôle d'exécutant, reste parfaitement substituable ; ce que le principe condamne, c'est un parent qui promet trop en logeant chez lui des attributs spécifiques à un seul type, ce qui est une faute de conception et non une propriété de TPT. TPC, lui, ne respecte pas ce principe, il le rend sans objet, puisqu'il supprime le supertype auquel on aurait substitué. Ce que TPC honore n'est donc pas le principe de substitution mais la préférence plus ancienne de Liskov, présente dès CLU, pour la composition plutôt que pour l'héritage : deux types concrets reliés par une référence étiquetée, sans hiérarchie parent-enfant.
...
**TPH** est philosophiquement condamné dans un contexte FAIR. Son problème n'est pas les colonnes NULL (PostgreSQL les gère efficacement), c'est que la signification de chaque ligne dépend d'un discriminant dont la sémantique est implicite. Un export CSV mélange des colonnes qui ne coexistent jamais sans que le lecteur puisse le savoir sans documentation. La localité sémantique est perdue, une ligne ne s'interprétant plus sans documentation externe, ce que l'exigence de réutilisabilité de FAIR sanctionne. Les contraintes d'intégrité interne à chaque type (tel attribut obligatoire pour tel type) s'accumulent sous forme de règles implicites fragiles.

**TPT** est la pire des trois stratégies pour l'export. Elle répartit les attributs d'une entité sur plusieurs tables : un export de la table parente ne donne qu'un squelette incompréhensible sans jointure. Elle impose une jointure systématique à chaque lecture d'un agent complet, un coût cumulé réel. Elle crée un point de couplage entre tous les types via la table parente, rigidifiant les migrations. Elle n'est en revanche légitime que si la table parente reste mince ; dès qu'on lui fait porter des attributs propres à un seul type, elle promet une communauté qui n'existe pas.

**TPC** a un coût technique concret et honnête. La FK native est perdue : PostgreSQL ne peut pas poser une contrainte de clé étrangère conditionnelle selon la valeur du discriminant. Les requêtes transversales sur tous les types nécessitent des `UNION ALL` verbeux. Le risque d'incohérence entre le discriminant et l'identifiant existe si l'API n'est pas le seul point d'écriture. Ces coûts sont réels mais circonscrits et mitigables. En contrepartie, TPC est la seule stratégie qui rend la sémantique auto-portée par les données : le discriminant `agentType` est lisible sur chaque ligne de chaque export, sans documentation externe. C'est ainsi qu'il résout en partie le problème de fond : il ne donne pas au moteur un vrai type somme, mais il place la disjonction, via `agentType`, là où un lecteur la trouve, sur la ligne elle-même.
...
TPC est le bon choix lorsque les types cibles sont ontologiquement distincts (ils ne partagent pas d'essence commune, seulement un rôle fonctionnel dans un contexte précis), que leurs attributs spécifiques sont nombreux et structurellement différents, que la sémantique doit rester lisible localement, une ligne exportée devant s'interpréter seule, ce que FAIR motive sans en être la seule raison, que les types constituent un ensemble fermé ou lentement évolutif, et que le modèle utilise déjà le pattern polymorphique ailleurs.
