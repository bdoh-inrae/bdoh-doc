...
| `KeywordAssignment` | Mots-clés thématiques                    | Observatory, Site, Station, TimeSerie, TransformedTimeSerie, TimeSeriesBundle, Property |
...
| `discipline`    | 1           | Famille majeure (en discussion)        | `hydrology` \| `chemistry` \| `meteorology` \| `sediment` \| `microbiology` \| `general` |
| `theme`         | 0..1        | Famille mineure (en discussion)        | `metals` \| `nutrients` \| `bacterialDiversity` \| `bacterialAbundance` \| `radionuclides` \| `pesticides` \| `pharmaceuticals` \| `general` |
| `samplingMedium`| 0..1        | Milieu intrinsèque (en discussion)     | `air` \| `surfaceWater` \| `groundwater` \| `soil` \| `sediment` \| `poreWater` \| `atmosphere` |
...
### Keyword
> Mot-clé thématique pour les catalogues et la découverte.
...
### KeywordAssignment
> Lien entre un keyword et une ressource -- pattern polymorphique.
Utilisé par : Observatory, Site, Station, TimeSerie, TransformedTimeSerie,
             TimeSeriesBundle, Property (via resourceType + resourceId)
Note : remplace les champs keyword 0..* sur chaque entité.
       Une ressource peut avoir autant de keywords que nécessaire.

| Champ          | Cardinalité | Définition                          | Valeurs possibles                                                                              |
|----------------|-------------|-------------------------------------|----------------------------------------------------------------------------------|
| `id`           | 1           | Identifiant technique, clé primaire | uuid                                                                             |
| `resourceType` | 1           | Type de ressource ciblée            | `Observatory` \| `Site` \| `Station` \| `TimeSerie` \| `TransformedTimeSerie` \| `TimeSeriesBundle` \| `Property` |
| `resourceId`   | 1           | UUID de la ressource ciblée         | uuid                                                                             |
...
| `codeType`     | 1           | Type d'identifiant                      | `uri` \| `doi` \| `orcid` \| `ror` \| `sandre` \| `wigos` \| `igsn` \| `other` |
...
| `type`                  | 1           | Type de contrôle                          | `independent_measure` \| `cross_validation` \| `reference_gauge`|
