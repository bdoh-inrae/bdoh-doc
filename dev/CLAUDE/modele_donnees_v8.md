# Modèle de données BDOH v8 -- Métadonnées des entités
...
TransformationBatch.inputSeries, Platform.timeseries) deviennent des endpoints bidirectionnels.
...
| `Memory`            | Notes, événements, photos                | Observatory, Site, Station, Sensor, Equipment, Platform, TimeSerie, TransformedTimeSerie, Deployment, Project, TransferFunction |
| `Responsibility`    | Rôles d'acteurs sur des ressources       | Observatory, Site, Station, Platform, TimeSerie, Project, TransferFunction, Equipment, Sensor |
...
- `SamplingFeature` (operator) : `person | machine`
...
| `platform_timeserie`               | Platform ↔ TimeSerie               |
...
──────────────────────         ──────────────────────────────────────
Datastream                  ←→ TimeSerie (via HistoricalDatastream)
Observation (raw)           ←→ ValidatedObservation (validée)
Station = Thing STA            Station (enrichie, métadonnées métier)
Sensor  = Sensor STA           Sensor (enrichi)
Property = ObservedProperty    Property (enrichie, discipline, thème...)
                               Platform (regroupement spatial de capteurs)
                               Deployment (acte de déploiement temporalisé)
```
...
Utilisé par : pattern TPC agent (agentType='person') sur ValidationBatch, TransformationBatch,
             ObservationBatch, TransferFunctionBatch, Memory, SamplingFeature.
...
| `organization` | 0..* →Org   | Employeur / labo de rattachement    | → Organization[] via person_organization  |
...
## 4bis. DONNÉES BRUTES (couche IoT STA 1.1)

### Datastream
> Flux de données brutes issu d'un unique capteur sur une station -- couche IoT STA.
...
Note : flux de données brutes pour un unique Thing + Sensor + ObservedProperty.
       ...
       plutôt que le resultType SWE-Common de STA 2.0 -- plus simple
...
```
Architecture instrumentation BDOH v8

Cas station fixe avec capteur individuel :

  Station (ancrage géographique permanent)
    └── Datastream → Sensor → HistoricalDatastream → TimeSerie

Cas bouée de lac (Platform multi-capteurs) :

  Station (bord du lac)
    └── Deployment [2020-présent]
          anchorType='station', anchorId=uuid_station
          └── Platform "Ligne verticale lac nord"
                platform_timeserie →
                  ├── TimeSerie T-0.5m    → HistoricalDatastream [DS-A 2020-2022] [DS-B 2022-...]
                  │                            deploymentDepth=-0.5m, depthReference=surfaceRelative
                  ├── TimeSerie O2-1.5m   → HistoricalDatastream [DS-C 2020-...]
                  │                            deploymentDepth=-1.5m
                  ├── TimeSerie Turb-5m   → HistoricalDatastream [DS-D 2020-2021] [DS-E 2021-...]
                  │                            deploymentDepth=-5.0m
                  └── TimeSerie Pos-capteur → HistoricalDatastream [DS-pression 2020-...]
                        (position réelle mesurée en continu -- post-traitement géospatial)

Cas drone (campagne mobile) :

  Site "Bassin versant Yzeron"
    └── Deployment [2024-05-10 / 2024-05-12]
          anchorType='site', anchorId=uuid_site
          location propre via HistoricalLocation (trajectoire)
          └── Platform "Bateau drone Yzeron"
                platform_timeserie →
                  └── TimeSerie Cond-surface → HistoricalDatastream [DS-conductivité]
```

---

### Platform
> Entité physique instrumentale regroupant des capteurs co-localisés -- bouée, ligne verticale, drone, sonde multi-paramètres.
Aligné avec : OGC CS API System/Platform, SOSA/SSN sosa:Platform, SensorML Platform
Utilisé par : Deployment (platform), platform_timeserie (platformId)
Relations inverses (requêter par resourceType='Platform') :
  Memory, Responsibility, Identifier, KeywordAssignment
Note : entité physique avec une identité propre, indépendante de son contexte de déploiement.
       Une Platform peut être réutilisée, déplacée, reconfigurée.
       Distinct de Deployment (l'acte de la poser quelque part à un moment donné).
       Les TimeSeries produites par ses capteurs sont liées via platform_timeserie.
       Type via KeywordAssignment (keywordType='platformType') :
       valeurs courantes : buoy, verticalChain, drone, multiProbe, weatherStation, mooring

| Champ         | Cardinalité | Définition                          | Valeurs possibles                   |
|---------------|-------------|-------------------------------------|-------------------------------------|
| `id`          | 1           | Identifiant technique, clé primaire | uuid                                |
| `code`        | 1           | Slug unique globalement             | "ligne-verticale-lac-nord"          |
| `name`        | 1           | Nom de la plateforme                | "Ligne verticale lac nord"          |
| `description` | 0..1        | Description libre                   |                                     |
| `status`      | 1           | État de la plateforme               | `active` \| `inactive` \| `retired` |

---

### Deployment
> Acte de déployer une Platform dans un contexte spatial et temporel donné -- lien entre Platform et territoire.
Aligné avec : OGC CS API Deployment, SOSA/SSN, SensorML DeploymentProperty
Utilisé par : Platform (via deployment.platform)
Relations inverses (requêter par resourceType='Deployment') :
  Memory, Identifier, HistoricalLocation
Note : porte le contexte de déploiement -- où, quand, rattaché à quoi.
       anchorType + anchorId : pattern TPC -- Station ou Site.
       Un Deployment sans anchorType est une plateforme autonome (drone en mission libre).
       La trajectoire d'un drone est tracée via HistoricalLocation sur le Deployment.
       Si la Platform est déplacée sur un autre site, c'est un nouveau Deployment.
       Plusieurs Deployments successifs d'une même Platform sont possibles.

| Champ        | Cardinalité | Définition                               | Valeurs possibles                   |
|--------------|-------------|------------------------------------------|-------------------------------------|
| `id`         | 1           | Identifiant technique, clé primaire      | uuid                                |
| `code`       | 1           | Slug unique globalement                  | "dep-lac-nord-2020"                 |
| `name`       | 1           | Nom du déploiement                       | "Déploiement ligne verticale 2020"  |
| `platform`   | 1 →Plat     | Platform déployée                        | → Platform                          |
| `anchorType` | 0..1        | Type d'ancrage territorial               | `station` \| `site`                 |
...
### platform_timeserie
> Lien many-to-many entre une Platform et les TimeSeries produites par ses capteurs.
Note : regroupe spatialement les TimeSeries co-localisées sur une même Platform.
       Pas de validFrom/validTo -- TimeSerie porte déjà ses dates de début et fin.
       La continuité temporelle de chaque TimeSerie est dans HistoricalDatastream.

| Champ         | Cardinalité | Définition           |
|---------------|-------------|----------------------|
| `platformId`  | 1 →Plat     | Platform parente     |
| `timeserieId` | 1 →TS       | TimeSerie membre     |
...
Utilisé par : Datastream (sensor), ControlObservation (sensor)
Relations inverses (requêter par resourceType='Sensor') :
  Responsibility, Identifier, Memory
Note : instrument de mesure indépendant de tout contexte d'utilisation.
       Un Sensor produit exactement un Datastream à la fois (règle métrologique et STA).
...
Utilisé par : SamplingFeature (equipmentId -- point ouvert)
...
Note : matériel de collecte terrain ou équipement fixe installé à poste.
       ...
       Point ouvert : lien avec SamplingFeature à revoir (une Platform peut aussi faire un prélèvement).
...
Note : lie une TimeSerie à ses Datastreams sources successifs dans le temps.
       ...
       Porte la position nominale du capteur (deploymentDepth) -- position de référence
       pour le post-traitement géospatial.
       Si la position varie en continu (bouée, ligne verticale mobile), une TimeSerie
       dédiée à la position est créée et liée à la même Platform via platform_timeserie.
...
| `deploymentDepth` | 0..1        | Position nominale du capteur en mètres | "-1.5"                                                       |
...
Utilisé par : ValidatedObservation (timeSerie), ControlObservation (timeSerie),
             ...
             platform_timeserie (timeserieId)
...
Note : porte tout ce qui est fixe et commun à toute la série.
       ...
       Le regroupement spatial avec d'autres séries co-localisées est via platform_timeserie.
...
Note : acte de prélèvement terrain. Présent pour les séries de type lab_sample.
       ...
       agentType + agentId : pattern TPC -- technicien terrain (person) dans le cas
       standard, préleveur automatique (machine) pour les préleveurs automatiques ISCO.
       Keywords attendus via KeywordAssignment (voir KeywordRequirement) :
...
| `agentType`           | 0..1        | Type d'agent ayant effectué le prélèvement    | `person` \| `machine`         |
| `agentId`             | 0..1        | UUID de la Person ou Machine                  | uuid                          |
